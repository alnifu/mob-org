...
import { Text, Avatar, Button, Card, Divider, List, ActivityIndicator, TextInput, Portal, Modal } from 'react-native-paper';
...
export const ProfileScreen = ({ navigation }: any) => {
  ...
  // Primary keys must be included in values to use upsert." so if the "id" column is your primary key then it must be present in the query.
  const uploadProfilePicture = async (uri: string) => {
    try {
      ...
      // Use member's ID as the filename
      ...
      // Update member record with the path (not the full URL)
      ...
      <Card style={styles.card} mode="outlined">
        <Card.Content>
          ...
          <List.Section>
            ...
            <List.Item
              title="Last Updated"
              description={member ? format(new Date(member.updated_at), 'MMMM dd, yyyy') : 'N/A'}
              left={props => <List.Icon {...props} icon="update" />}
            />
            <List.Item
              title="Organizations"
              description={member?.organization_ids?.length ? `${member.organization_ids.length} organizations` : 'No organizations'}
